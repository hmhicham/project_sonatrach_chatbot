...
function MapControls({ onCenterMap }) {
  ...
  return (
    <div className="map-controls">
      <button onClick={handleZoomIn} className="map-control-button" title="Zoom In" aria-label="Zoom In">
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
        </svg>
      </button>
      <button onClick={handleZoomOut} className="map-control-button" title="Zoom Out" aria-label="Zoom Out">
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 12h16" />
        </svg>
      </button>
      <button onClick={onCenterMap} className="map-control-button" title="Center Map" aria-label="Center Map">
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12h-6m-3-9v6m-9 3h6m3 9v-6" />
        </svg>
      </button>
...
function DrawPolygon({ onCreated, entityType, canDrawPRM }) {
  const map = useMap();
  console.log('DrawPolygon: Map instance:', map, 'canDrawPRM:', canDrawPRM);
  ...
  useEffect(() => {
    ...
    map.pm.addControls({
      position: 'topright',
      drawPolygon: entityType !== 'WELL' && canDrawPRM,
      drawMarker: entityType === 'WELL' && canDrawPRM,
      drawPolyline: false,
      drawRectangle: false,
      drawCircle: false,
      drawCircleMarker: false,
    ...
    const handleCreate = (e) => {
      if (!canDrawPRM) {
        alert('Only carto users can draw shapes.');
        return;
      }
      try {
        if (e.shape === 'Polygon') {
  ...
  }, [map, onCreated, entityType, canDrawPRM]);
...
function Home({ username, isManager }) {
  ...
  const [subordinates, setSubordinates] = useState([]);
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [canDrawPRM, setCanDrawPRM] = useState(false); // Track 'carto' group membership
  ...
  useEffect(() => {
    ...
    const checkAuthStatus = async () => {
      try {
        ...
        console.log('Auth status:', authResponse.data);
        setIsAuthenticated(authResponse.data.is_authenticated);
        const groups = authResponse.data.groups || [];
        console.log('Groups:', groups);
        setCanDrawPRM(groups.includes('carto')); // Set based on 'carto' group
        console.log('canDrawPRM:', canDrawPRM);
        if (!authResponse.data.is_authenticated) {
          alert('You are not authenticated. Please log in.');
          window.location.href = 'http://127.0.0.1:8000/login/';
        }
  ...
  const handleCenterMap = () => {
    const map = mapRef.current;
    if (map) map.setView([33.5, 2.5], 5);
    else console.warn('handleCenterMap: Map instance is not available');
  };
  ...
  const addShapeFromCoordinates = () => {
    if (!canDrawPRM) {
      alert('Only carto users can enter coordinates for shapes.');
      return;
    }
    if (!lat || !lng) {
      console.error('Latitude or Longitude is empty');
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.data.length > 0) {
        const result = response.data[0];
        const map = mapRef.current;
        if (map && result.position) map.setView(result.position, 10);
        else if (map && result.positions) map.fitBounds(result.positions);
      }
    } catch (error) {
      console.error('Search error:', error);
    }
    ...
    <div>
      {/* <header>
        <div className="container">
          <h1>Plateforme Sonatrach</h1>
          <p>
            Bienbbvenue, {username} | <button onClick={handleLogout}>Déconnexion</button>
            {isManager && (
              <>
                {' | '}
                <span>Manager</span>
      ...
      <main>
        <div className="container">
          <div className="toolbar">
            ...
            <button onClick={() => setShowLegend(!showLegend)} className="legend-button">
              {showLegend ? 'Masquer la légende' : 'Afficher la légende'}
            </button>
          </div>
          ...
          <div className="map-container">
            ...
            >
                ...
                concessions
                  ...
                  .map((concession) => (
                    <Polygon
                      ...
                      pathOptions={{ color: '#FF9800' }}
                      eventHandlers={{
                        click: () => handlePolygonClick(concession.name),
                      }}
                    >
                      <Popup>
                        <div>{concession.name}</div>
              ...
              {layers.puits &&
                wells
                  .filter(
                    (well) => well.position && Array.isArray(well.position) && well.position.length === 2
                  )
                  .map((well) => (
                    <Marker key={well.sigle} position={well.position}>
                      <Popup>
              ...
              {drawnShapes.map((shape, index) =>
                shape.positions && Array.isArray(shape.positions) && shape.positions.length > 0 &&
                shape.positions.every((coord) => Array.isArray(coord) && coord.length === 2) ? (
                  <Polygon key={index} positions={shape.positions} pathOptions={{ color: 'cyan' }}>
                    <Popup>
                      <div>New Shape</div>
                      <div>Created by: {shape.created_by || 'Unknown'}</div>
              ...
              <DrawPolygon onCreated={handleDrawCreated} entityType={entityType} canDrawPRM={canDrawPRM} />
              <MapControls onCenterMap={handleCenterMap} />
            </MapContainer>
            ...
            {showCoordinateInput && canDrawPRM && (
              <div className="modal-overlay">
                <div className="modal">
                  <div className="modal-header">
                    <h2>Entrer des coordonnées</h2>
